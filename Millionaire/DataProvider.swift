...
class DataProvider {
   ...
   static func storeFetched(questions: [JSON]) {
        let fetchRequest = NSFetchRequest(entityName:Question.className)
        for json in questions {
            ...
            do{
                let fetchedResults = try CoreDataHelper.instance.context.executeFetchRequest(fetchRequest) as? [Question];
    ...
    private static let LAST_SYNC_DATE = "LAST_SYNC_DATE";
    ...
    static func deleteExtraData(){
        ...
        do{
            let arrayToDelete = try CoreDataHelper.instance.context.executeFetchRequest(request) as? [Question];
    ...
    static func synchronize(finishCallback:(result: String)->Void){
        
        func fetchingDataCallback(json: JSON?, error: NSError?){
            if error != nil {
                finishCallback(result: "fail after auth");
            } else if let j = json {
                if let err = j["error"].string {
                    finishCallback(result: "error parameter \(err)");
                }
                if let response = j["response"].array {
                    ...
                    finishCallback(result: "got data");
                    
                }else{
                    finishCallback(result: "no response parameter");
                }
            }
        ...
        func resetingDataCallback(json: JSON?, error: NSError?){
            Question.deleteAll();
            fetchingDataCallback(json, error: error);
        }
        
        func authCallback(json: JSON?, error: NSError?){
       //    print("auth json "+String(json))

            if error != nil {
             finishCallback(result: "auth error");
            } else if let j = json {
             
                if let keyDate = j["date"].int {
                  
                 //   print("local \(getLocalSyncDate()) key \(Int(keyDate)) curr \(currentTimeMillis()) jdate \(j["date"])");
                    
                    if(getLocalSyncDate()<Int(keyDate)){
                        print("just update")
                        APIService.fetchData(fromSavedUrl: ServerSettings.host.URLByAppendingPathComponent( ServerSettings.synchronizeQuestionsPath), parameters: ["hash":encrypt(String(keyDate))], callback: fetchingDataCallback);
                        
                        setLocalSyncDate(keyDate)
                    }else{
                      finishCallback(result: "questions is still actual");
                        
                    }
                }else if let lastDeleteDate = j["delete_date"].int{
                    print("reset all")
                    APIService.fetchData(fromSavedUrl: ServerSettings.host.URLByAppendingPathComponent( ServerSettings.synchronizeQuestionsPath), parameters: ["hash":encrypt(String(lastDeleteDate))], callback: resetingDataCallback);
                    setLocalSyncDate(lastDeleteDate)

                    
                }
                else {
                    finishCallback(result: "no date parameter");
                }
        ...
        APIService.fetchData(fromSavedUrl: ServerSettings.host.URLByAppendingPathComponent( ServerSettings.synchronizeQuestionsPath), parameters: ["date":getLocalSyncDate()], callback: authCallback)
    ...
    static func encrypt(date:String)->String{
    ...
    static func getLocalSyncDate()->Int{
         return NSUserDefaults.standardUserDefaults().integerForKey(LAST_SYNC_DATE)
    }
    
    static func setLocalSyncDate(syncDate: Int){
